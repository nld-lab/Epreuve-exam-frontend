...
import { ArrowLeft, Download, Eye, Calendar, BookOpen, FolderTree } from "lucide-react";
...
function InfoRow({ icon: Icon, label, value }: { icon: typeof Calendar; label: string; value?: string | null }) {
  return (
...
export function EpreuveDetailPage() {
  const { id } = useParams();
  const { data: ep, isLoading, isError } = useEpreuve(id ? Number(id) : undefined);
        ...
        <Card>
          <CardHeader>
            <div className="flex items-start justify-between gap-2">
              ...
              <Badge variant={ep.session === "RATTRAPAGE" ? "destructive" : "secondary"}>
          ...
          <CardContent className="space-y-3">
            ...
            <InfoRow icon={FolderTree} label="Filière" value={ep.filiere?.nom} />
            <InfoRow icon={Calendar} label="Année d'étude" value={ep.annee?.libelle} />
            <InfoRow icon={Calendar} label="Année acad." value={ep.anneeAcademique} />
